...
async fn main() -> Result<(), lexopt::Error> {
    ...
    while let Some(arg) = parser.next()? {
        match arg {
            ...
            lexopt::Arg::Short('h') | lexopt::Arg::Long("help") => {
                ...
                println!("\nOptions:");
                println!("  -s, --split <N>     Number of concurrent threads (Default: 1, Max: 99)");
    ...
    if let Some(url) = url {
        ...
        let filename = out.unwrap_or_else(|| resolved.filename.unwrap_or_else(|| "download.bin".to_string()));
        ...
        if log {
            ...
            println!("  \x1b[1;33m📏 Size:      \x1b[0;37m{} \x1b[90m({})\x1b[0m", format_bytes(total_size), file_type);
            println!("  \x1b[1;33m⚡ Threads:   \x1b[0;37m{} Workers", split);
            println!("  \x1b[1;33m📂 Target:    \x1b[0;37m{}", dir);
            println!("  \x1b[1;33m⏯️  Resumable: \x1b[0;37m{}", if resumable { "\x1b[32mYes\x1b[0m" } else { "\x1b[31mNo\x1b[0m" });
            println!("  \x1b[90m───────────────────────────────────────────\x1b[0m\n");
        ...
        match task.start(token.clone()).await {
            Ok((content_length, actual_threads, _file_type, _is_resumable, mut progress_rx)) => {
                ...
                while let Some((worker_id, bytes_chunk)) = progress_rx.recv().await {
                    ...
                    if log && (now.duration_since(last_render).as_millis() >= 100 || completed == content_length) {
                        last_render = now;
                        let percent = (completed as f64 / content_length as f64) * 100.0;
                        let bar_width = 30;
                        let filled = (percent / 100.0 * bar_width as f64) as usize;
                        let bar = format!("\x1b[32m{:█<filled$}\x1b[90m{:░<empty$}\x1b[0m", "", "", filled = filled, empty = bar_width - filled);
                        ...
                        for (i, p) in thread_progress.iter().enumerate() {
                            thread_status.push_str(&format!("\x1b[90m[\x1b[33mT{}:\x1b[37m{}\x1b[90m]\x1b[0m ", i + 1, format_bytes_compact(*p)));
                        }
    ...
    // Default Server mode
    println!("\x1b[1;34mStarting Pincer engine v{} ({})\x1b[0m", VERSION, BUILD_TYPE);
